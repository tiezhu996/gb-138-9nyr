...
- 症状照护与心理支持信息
- 资源、愿望清单和家属指南
- 心理压力自测档案：填写人署名后提交，服务端算分并给出建议；同一人同一天重复提交以最后一次为准，历史记录长期保存，可随时翻看并与更早的记录对比变化

## 自测档案接口

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/assessments?name=姓名` | 查询该填写人的全部自测档案（新的在前，第一条即上次总分） |
| POST | `/api/assessments` | 提交自测，请求体 `{"name":"姓名","answers":[0..3 共8个]}`；服务端校验、算分、归档 |

自测记录保存在 PostgreSQL 的 `assessment_records` 表中（`db_data` 数据卷持久化，重启不丢失）。
